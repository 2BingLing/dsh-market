...
> **v0.4.5 起**：确认安装时可勾选 **🛡 安全模式**——对于来源不明或敏感的插件，开启后**跳过零 LLM 直装**，强制 AI 子代理在安装前做供应链扫描：检查危险命令（`curl|sh` 管道执行 / 下载执行 / base64 混淆）、敏感信息收集回传（读取 API Key / Token 并外发）、配置篡改（覆盖 sandbox / approval / 权限配置）、来源存疑（新建低活跃仓库）等；**发现风险即中止、不执行任何安装命令**。安全模式消耗 token，普通可信插件可保持关闭走快速安装。背景：DSH 插件供应链曾曝极危漏洞（QVD-2026-57410，CVSS 9.8）。
...
- **持续收录** — 每天自动扫描 `dsh-plugin` / `dsh` 等 GitHub topic、社区精选列表，全量收录（当前 5572 个插件 + 整合包通道）
...
- **作者自述**：提交时附上「作者自述简介」，会展示在市场详情页的「作者自述」专区（带作者标识）——用你自己的话介绍插件，比自动简介更有温度。已收录插件可提交 `[数据修正]` issue 补写
