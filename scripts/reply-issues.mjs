...
function buildFixReply(r) {
  ...
  return [
    ...
    `✅ **已应用数据修正** \`${r.fullName}\``,
    ``,
    `- 你提交的作者自述 / 简介 / 安装命令修正已进入市场数据，**次日 06:00 更新**后可在 [DSH Market](https://dsh.market/) 查看（${listedUrl}）。`,
    `- 若你修正了**安装命令**：请顺手把仓库 README 的安装示例同步成新命令——安装命令以 README 解析为准，本 issue 关闭后管道会跟着 README 走。`,
    `- 类型等更复杂字段的修正，建议直接重新提交一个 \`[提交插件]\` issue（附新的仓库信息），会自动进入收录流程。`,
    ``,
