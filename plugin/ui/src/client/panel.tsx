            ...
            ...visible.map((p) => {
              ...
              return El('div', { key: p.id, className: styles.card },
                El('div', { className: styles.cardTop },
                  El('span', { className: styles.cardStars, style: { background: 'var(--mkt-purple-bg)', color: 'var(--mkt-purple)', borderRadius: 6, padding: '2px 8px', fontSize: 10 } }, 'PACK'),
                ...
                El('div', { style: { display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: rate >= 80 ? 'var(--mkt-rate-ok)' : rate >= 50 ? 'var(--mkt-rate-warn)' : 'var(--mkt-rate-danger)', padding: '4px 8px', borderRadius: 6, background: 'var(--mkt-bg)', marginBottom: 8 } },
                  `✓ ${ok}/${total} 条目可解析 · ${inMarket} 已在市场`,
                  El('b', {}, `${rate}%`),
                ),
                El('div', { className: styles.cardActions },
                  El('span', { style: { fontSize: 12, fontWeight: 600, color: 'var(--mkt-purple)' } }, `${p.scoreTotal} 实用分`),
                  El('button', {
                    className: `${styles.btn} ${styles.btnSm} ${styles.btnGhost}`,
