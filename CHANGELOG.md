...
- 环境要求写明 **DSH ≥ 0.1.5** / **Node ≥ 20**，并一句话说明面板入口自动适配与旧版维护模式（**0.5.0 起移除**）——保持简短，不堆版本依据。
- `plugin/ui/README.md`（npm 包页面）同步。
- 版本兼容的详细依据（四项前提逐版核对、`layout.selectPanel` 能力探测、双路注册约束等）放在 `docs/EXECUTION-GUIDE.md`，不进 README。
